...
<body>
    ...
    <?php
        foreach($data as $item){
            echo '<div class="form-video" onclick=watch('.$item['id_playlist'].')>
                <!-- link chanel of account -->
                <img src="assets/images/Thumbnails-img.jpg" alt="">
                
                <div class="summary-video">
                    <!-- click move to chanel -->
                    <div class="inf-video">
                        <h1 class="content-video">'.$item["name_playlist"].'</h1>
                    </div>
                </div>
            </div>';
        }
    ...
    <script>
        ...
        function watch(id){
            window.open("index.php?controller=User&action=getPlaylist&id=" + id, "_self");
        }
